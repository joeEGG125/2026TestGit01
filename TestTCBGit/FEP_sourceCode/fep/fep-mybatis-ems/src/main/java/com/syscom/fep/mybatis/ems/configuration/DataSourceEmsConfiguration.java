...
@Configuration
@MapperScan(
        basePackages = {DataSourceEmsConstant.PACKAGE_MAPPER, DataSourceEmsConstant.PACKAGE_EXT_MAPPER},
        annotationClass = Resource.class,
        sqlSessionTemplateRef = DataSourceEmsConstant.BEAN_NAME_SQL_SESSION_TEMPLATE,
        sqlSessionFactoryRef = DataSourceEmsConstant.BEAN_NAME_SQL_SESSION_FACTORY,
        lazyInitialization = "${" + DataSourceEmsConstant.CONFIGURATION_PROPERTIES_LAZY_INITIALIZATION + ":true}"
)
// @RefreshScope
public class DataSourceEmsConfiguration implements DataSourceEmsConstant, BeanOperationListener {
    ...
    public DataSource dataSource(@Qualifier(BEAN_NAME_HIKARI_CONFIG) HikariConfig hikariConfig) throws Exception {
        ...
        LogHelperFactory.getGeneralLogger().info(" ems username  : " + pkiConfig.getProperty(CONFIGURATION_PROPERTIES_USERNAME));
        LogHelperFactory.getGeneralLogger().info(" ems password  : ***" + Jasypt.decrypt(pkiConfig.getProperty(CONFIGURATION_PROPERTIES_PASSWORD)));
