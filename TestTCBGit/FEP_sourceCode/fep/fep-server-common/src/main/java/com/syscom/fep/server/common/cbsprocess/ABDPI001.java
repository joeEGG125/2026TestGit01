...
public class ABDPI001 extends ACBSAction {
	...
	public FEPReturnCode getCbsTita(String txType) throws Exception {
		...
		if(txType.equals("0")){ //檢核
			...
			cbstita.setMSG_CAT("10"); //財金REP MSGID
		...
		cbstita.setRESPONSE_CODE(feptxn.getFeptxnRepRc()); //正常才上送
		// ATM交易序號
		...
		if(txType.equals("0")){
			...
			cbstita.setICMEMO(feptxn.getFeptxnIcmark()); //IC卡備註欄 
	...
	private FEPReturnCode updateFEPTxn(AB_DP_O001 cbsTota) throws Exception {
    	...
    	//轉出帳號
    	//if(StringUtils.isNotBlank(cbsTota.getFROMACT())) {
    	//	feptxn.setFeptxnMajorActno(cbsTota.getFROMACT());
    	//}
    	//轉入帳號
    	//if(StringUtils.isNotBlank(cbsTota.getTOACT())) {
    	//	feptxn.setFeptxnTrinActno(cbsTota.getTOACT());
    	//}
    	//實際交易金額(扣除手續費)
    	if(cbsTota.getDPAMT() != null && cbsTota.getDPAMT().compareTo(BigDecimal.ZERO ) > 0) {
    		feptxn.setFeptxnTxAmt(cbsTota.getDPAMT());
