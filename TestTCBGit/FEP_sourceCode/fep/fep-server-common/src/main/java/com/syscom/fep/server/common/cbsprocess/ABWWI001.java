...
public class ABWWI001 extends ACBSAction {
	...
	public FEPReturnCode getCbsTita(String txType) throws Exception {
		...
		cbstita.setFROMACT(feptxn.getFeptxnTrk2().substring(0,16)); //取二軌前16碼
		...
		cbstita.setEMVAUTH(feptxn.getFeptxnAuthcd()); //授權碼
		cbstita.setTRK2DATA(atmReq.getTRK2()); //ATM上送二軌資料
		cbstita.setOGSLAMT(feptxn.getFeptxnTxAmtSet()); //清算金額
		getLogContext().setRemark("TxAmtAct: " + feptxn.getFeptxnTxAmtAct() +
				",TxAmt: " + feptxn.getFeptxnTxAmt() + ",TxAmtSet: " + feptxn.getFeptxnTxAmtSet());
		this.logMessage(getLogContext());

		//清算幣別，取紀錄在INTLTXN 的ISO幣別碼
		Intltxn intltxn = intltxnMapper.selectByPrimaryKey(feptxn.getFeptxnTxDate(), feptxn.getFeptxnEjfno());
		cbstita.setCCDSETTLE(intltxn.getIntltxnSetCur()); //清算幣別

		//預借現金交易類別
		if(StringUtils.equals(feptxn.getFeptxnRepRc().substring(0, 3), "400")) {
			cbstita.setTRANSTYPE(mINTLTXN.getIntltxnTranstype()); 
		}
	...
	private FEPReturnCode updateFEPTxn(AB_WW_O001 cbsTota) throws Exception {
    	...
    	if(StringUtils.isNotBlank(cbsTota.getFROMBANK())) {
