        ...
        try {
            ...
            if ("LE".equals(fscode) || "LF".equals(fscode)) {
                ...
                if (StringUtils.isNotBlank(atmReqbody.getINDATE())
                    ...
                    int receiveDiffseconds = MathUtil.safeGet((int) ((end - receive) / 1000));
                    ...
                    if ((diffseconds > 90 )  && "RQ".equals(atmReqbody.getTXNTYPE().trim())) {
                        ...
                        if (AEIPCRAC.length() > 16 || AEIPYAC.length() > 16) {
                            SEND_VA_GeneralTrans_RS rs = new SEND_VA_GeneralTrans_RS();
                            SEND_VA_GeneralTrans_RS.SEND_VA_GeneralTrans_RS_Body rsbody = new SEND_VA_GeneralTrans_RS.SEND_VA_GeneralTrans_RS_Body();
                            SEND_VA_GeneralTrans_RS.SEND_VA_GeneralTrans_RS_Body_MsgRs msgrs = new SEND_VA_GeneralTrans_RS.SEND_VA_GeneralTrans_RS_Body_MsgRs();
                            SEND_VA_GeneralTrans_RS.SEND_VA_GeneralTrans_RS_Body_MsgRs_Header header = new SEND_VA_GeneralTrans_RS.SEND_VA_GeneralTrans_RS_Body_MsgRs_Header();
                ...
                if (StringUtils.isNotBlank(atmReqbody.getINDATE())
                    ...
                    timeStart = (atmReqbody.getINDATE().length() == 7 ? String.valueOf(Integer.valueOf(atmReqbody.getINDATE()) + 19110000) : atmReqbody.getINDATE()) + atmReqbody.getINTIME();
...
//	}
    ...
    private boolean setNBMsgid() throws Exception {
...
//                        nbData.getAtmtxObject().getRequest().setPICCBI55("202020202020202020202020202020");
    ...
    private boolean setVAMsgid() throws Exception {
        RCV_VA_GeneralTrans_RQ.RCV_VA_GeneralTrans_RQ_Body_MsgRq_SvcRq vaSvcRq = nbData.getTxVafepObject().getRequest().getBody().getRq().getSvcRq();
        RCV_VA_GeneralTrans_RQ.RCV_VA_GeneralTrans_RQ_Body_MsgRq_Header vaHeader = nbData.getTxVafepObject().getRequest().getBody().getRq().getHeader();
        String txntype = vaSvcRq.getTXNTYPE().trim();
        String sysstatHbkno = SysStatus.getPropertyValue().getSysstatHbkno();
        if ("IQ".equals(txntype) || "RV".equals(txntype)) {
            // RUN  NBINQ
            nbData.setMessageID("NBINQ");
            ...
            String aeipcrbk = vaSvcRq.getAEIPCRBK();
            if ("RQ".equals(txntype)) {
                switch (fscode) {
                    case "LE":
                        if ("03".equals(vaSvcRq.getAEIPYTP())) {
                            ...
                            if (npsunit != null) {
                                String bkno = npsunit.getNpsunitBkno().substring(0, 3);
                                if (StringUtils.equals(bkno, sysstatHbkno)) {
                                    nbData.setMessageID(vaSvcRq.getFSCODE() + vaSvcRq.getPCODE() + "0");
                                } else {
                                    nbData.setMessageID(vaHeader.getMSGID());
                                }
                            } else {
                                //如果查無NPSUNIT 進自行AA 再進行一次判斷做剔退
                                nbData.setMessageID(vaSvcRq.getFSCODE() + vaSvcRq.getPCODE() + "0");
                            }
                        } else {
                            if (StringUtils.equals(aeipcrbk, sysstatHbkno)) {
...
//                        nbData.getAtmtxObject().getRequest().setPICCBI55(vaSvcRq.getICMARK());
//                        nbData.getAtmtxObject().getRequest().setPICCTAC("0000" + vaSvcRq.getIC_TAC());
                        break;
                    case "LF":
                        String outbank = nbData.getTxVafepObject().getRequest().getBody().getRq().getSvcRq().getAEIPCRBK();
                        String inbank = nbData.getTxVafepObject().getRequest().getBody().getRq().getSvcRq().getAEIPYBK();
//                        nbData.getAtmtxObject().getRequest().setPICCBI55("202020202020202020202020202020");
                        if (StringUtils.equals(outbank, sysstatHbkno) && StringUtils.equals(outbank, inbank)) {
                            nbData.setMessageID(vaSvcRq.getFSCODE() + vaSvcRq.getPCODE() + "0");
                        } else {
                    ...
                    default:
                        outbank = nbData.getTxVafepObject().getRequest().getBody().getRq().getSvcRq().getAEIPCRBK();
                        inbank = nbData.getTxVafepObject().getRequest().getBody().getRq().getSvcRq().getAEIPYBK();
//                        nbData.getAtmtxObject().getRequest().setPICCBI55("202020202020202020202020202020");
                        if (StringUtils.equals(outbank, sysstatHbkno) && StringUtils.equals(outbank, inbank)) {
                            nbData.setMessageID(vaSvcRq.getFSCODE() + vaSvcRq.getPCODE() + "0");
                        } else {
