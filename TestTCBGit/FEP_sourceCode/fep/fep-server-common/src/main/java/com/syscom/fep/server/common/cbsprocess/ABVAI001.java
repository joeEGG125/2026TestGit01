...
//		cbstita.setIMS_TRANS("MFEPEA00");
		...
		if (DbHelper.toShort(false).equals(feptxn.getFeptxnFiscFlag())) {
			...
			if(StringUtils.equals(atmRequest.getBody().getRq().getSvcRq().getFSCODE(), "LF")){
				cbstita.setMSG_CAT(AEIPYTP);
			} else {
				cbstita.setMSG_CAT(atmRequest.getBody().getRq().getSvcRq().getTXNTYPE());
			}
...
//		cbstita.setMSG_CAT(atmRequest.getBody().getRq().getSvcRq().getTXNTYPE());
		...
		if(VACATE.equals("10")){
			...
			if(StringUtils.equalsAny(AEIPYTP, "00","01")){
				cbstita.setCARDTYPE("K");
			}else{
				cbstita.setCARDTYPE("N");
			}
...
//		}
		...
		if("02".equals(VACATE)) {
			...
			cbstita.setEIPYAC2(atmRequest.getBody().getRq().getSvcRq().getSENDDATA().getAEIPYAC2());
		}
