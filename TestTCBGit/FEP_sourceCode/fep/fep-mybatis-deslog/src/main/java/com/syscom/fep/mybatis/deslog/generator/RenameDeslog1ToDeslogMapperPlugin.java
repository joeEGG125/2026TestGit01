...
import com.syscom.fep.mybatis.deslog.vo.MybatisElement;
...
public class RenameDeslog1ToDeslogMapperPlugin extends PluginAdapter {
	...
	public void initialized(IntrospectedTable introspectedTable) {
		if (MybatisConstant.TABLE_NAME_DESLOG1.equals(introspectedTable.getFullyQualifiedTable().getIntrospectedTableName())) {
			...
			mybatis3JavaMapperType = StringUtils.replace(mybatis3JavaMapperType, MybatisConstant.ENTITY_NAME_DESLOG1, MybatisConstant.REPLACE_ENTITY_NAME_FOR_DESLOG1);
			...
			mybatis3XmlMapperFileName = StringUtils.replace(mybatis3XmlMapperFileName, MybatisConstant.ENTITY_NAME_DESLOG1, MybatisConstant.REPLACE_ENTITY_NAME_FOR_DESLOG1);
			introspectedTable.setMyBatis3XmlMapperFileName(mybatis3XmlMapperFileName);
			// change module name
			String baseRecordType = introspectedTable.getBaseRecordType();
			baseRecordType = StringUtils.replace(baseRecordType, MybatisConstant.ENTITY_NAME_DESLOG1, MybatisConstant.REPLACE_ENTITY_NAME_FOR_DESLOG1);
	...
	public boolean sqlMapSelectByPrimaryKeyElementGenerated(XmlElement element, IntrospectedTable introspectedTable) {
		...
		if (MybatisConstant.ENTITY_NAME_DESLOG1.equals(modelName)) {
			modelName = MybatisConstant.REPLACE_ENTITY_NAME_FOR_DESLOG1;
		}
		if (MybatisConstant.TABLE_NAME_DESLOG1.equals(introspectedTable.getFullyQualifiedTable().getIntrospectedTableName())) {
		...
		if (CollectionUtils.isNotEmpty(root.getElements())) {
			...
			for (VisitableElement element : root.getElements()) {
				if (element instanceof XmlElement) {
					...
					if (content.contains(MybatisConstant.TABLE_NAME_DESLOG1)) {
						...
						element = new TextElement(StringUtils.replace(content, MybatisConstant.TABLE_NAME_DESLOG1, MybatisConstant.REPLACE_TABLE_NAME_FOR_DESLOG1_INCLUDE_REFID));
					}
				}
				list.add(element);
			}
	...
	private VisitableElement ceateChooseTableNameSqlElement() {
		...
		for (int i = 1; i <= 7; i++) {
			...
			when.addElement(new TextElement(StringUtils.join(MybatisConstant.TABLE_NAME_PREFIX_DESLOG, tableNameSuffix)));
