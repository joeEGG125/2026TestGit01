...
@Configuration
@MapperScan(
        basePackages = {DataSourceDeslogConstant.PACKAGE_MAPPER, DataSourceDeslogConstant.PACKAGE_EXT_MAPPER},
        annotationClass = Resource.class,
        sqlSessionTemplateRef = DataSourceDeslogConstant.BEAN_NAME_SQL_SESSION_TEMPLATE,
        sqlSessionFactoryRef = DataSourceDeslogConstant.BEAN_NAME_SQL_SESSION_FACTORY,
        lazyInitialization = "${" + DataSourceDeslogConstant.CONFIGURATION_PROPERTIES_LAZY_INITIALIZATION + ":true}"
)
// @RefreshScope
public class DataSourceDeslogConfiguration implements DataSourceDeslogConstant, BeanOperationListener {
    ...
    public DataSource dataSource(@Qualifier(BEAN_NAME_HIKARI_CONFIG) HikariConfig hikariConfig) throws Exception {
        ...
        LogHelperFactory.getGeneralLogger().info(" deslog username  : " + pkiConfig.getProperty(CONFIGURATION_PROPERTIES_USERNAME));
        LogHelperFactory.getGeneralLogger().info(" deslog password  : ***" + Jasypt.decrypt(pkiConfig.getProperty(CONFIGURATION_PROPERTIES_PASSWORD)));
