...
@Configuration
@MapperScan(
        basePackages = {DataSourceEncConstant.PACKAGE_MAPPER, DataSourceEncConstant.PACKAGE_EXT_MAPPER},
        annotationClass = Resource.class,
        sqlSessionTemplateRef = DataSourceEncConstant.BEAN_NAME_SQL_SESSION_TEMPLATE,
        sqlSessionFactoryRef = DataSourceEncConstant.BEAN_NAME_SQL_SESSION_FACTORY,
        lazyInitialization = "${" + DataSourceEncConstant.CONFIGURATION_PROPERTIES_LAZY_INITIALIZATION + ":true}"
)
// @RefreshScope
public class DataSourceEncConfiguration implements DataSourceEncConstant, BeanOperationListener {
    ...
    public DataSource dataSource(@Qualifier(BEAN_NAME_HIKARI_CONFIG) HikariConfig hikariConfig) throws Exception {
        ...
        LogHelperFactory.getGeneralLogger().info(" enc username  : " + pkiConfig.getProperty(CONFIGURATION_PROPERTIES_USERNAME));
        LogHelperFactory.getGeneralLogger().info(" enc password  : ***" + Jasypt.decrypt(pkiConfig.getProperty(CONFIGURATION_PROPERTIES_PASSWORD)));
