...
import com.syscom.fep.mybatis.vo.MybatisElement;

public class RenameFeptxn01ToFeptxnMapperPlugin extends PluginAdapter {
    ...
    public void initialized(IntrospectedTable introspectedTable) {
        if (MybatisConstant.TABLE_NAME_FEPTXN01.equals(introspectedTable.getFullyQualifiedTable().getIntrospectedTableName())) {
            // change mapper name
            String mybatis3JavaMapperType = introspectedTable.getMyBatis3JavaMapperType();
            mybatis3JavaMapperType = StringUtils.replace(mybatis3JavaMapperType, MybatisConstant.ENTITY_NAME_FEPTXN01, MybatisConstant.REPLACE_ENTITY_NAME_FOR_FEPTXN01);
            ...
            mybatis3XmlMapperFileName = StringUtils.replace(mybatis3XmlMapperFileName, MybatisConstant.ENTITY_NAME_FEPTXN01, MybatisConstant.REPLACE_ENTITY_NAME_FOR_FEPTXN01);
            introspectedTable.setMyBatis3XmlMapperFileName(mybatis3XmlMapperFileName);
            // change module name
            String baseRecordType = introspectedTable.getBaseRecordType();
            baseRecordType = StringUtils.replace(baseRecordType, MybatisConstant.ENTITY_NAME_FEPTXN01, MybatisConstant.REPLACE_ENTITY_NAME_FOR_FEPTXN01);
        ...
        if (CollectionUtils.isNotEmpty(root.getElements())) {
            ...
            for (VisitableElement element : root.getElements()) {
                if (element instanceof XmlElement) {
                    ...
                    if (content.contains(MybatisConstant.TABLE_NAME_FEPTXN01)) {
                        // element = new TextElement(StringUtils.replace(content, MybatisConstant.TABLE_NAME_FEPTXN01, MybatisConstant.REPLACE_TABLE_NAME_FOR_FEPTXN01));
                        element = new TextElement(StringUtils.replace(content, MybatisConstant.TABLE_NAME_FEPTXN01, MybatisConstant.REPLACE_TABLE_NAME_FOR_FEPTXN01_INCLUDE_REFID));
                    }
                }
                list.add(element);
            }
    ...
    private VisitableElement ceateChooseTableNameSqlElement() {
        ...
        for (int i = 1; i <= 31; i++) {
            tableNameSuffix = StringUtils.leftPad(String.valueOf(i), 2, '0');
            XmlElement when = new XmlElement("when");
            when.addAttribute(new Attribute("test", StringUtils.join("tableNameSuffix == '", tableNameSuffix, "'")));
            when.addElement(new TextElement(StringUtils.join(MybatisConstant.TABLE_NAME_PREFIX_FEPTXN, tableNameSuffix)));
