...
public class OFTRequest extends ATMTextBase {
	//空白
	@Field(length = 4)
	private String _TA = "";
	...
	public void toGeneral(ATMGeneral atmGeneral) throws Exception {
		/*
		atmGeneral.getRequest().setTA(this.toAscii(this.getTA())); //空白
		atmGeneral.getRequest().setTXCD(this.toAscii(this.getTXCD())); //交易別
		atmGeneral.getRequest().setAtmverN(this.toAscii(this.getAtmverN())); //新舊電文註記
		atmGeneral.getRequest().setATMVER(this.toAscii(this.getATMVER())); //ATM版本日期
		atmGeneral.getRequest().setBRNO(this.toAscii(this.getBRNO())); //ATM所屬分行
		atmGeneral.getRequest().setWSNO(this.toAscii(this.getWSNO())); //ATM 代號
		atmGeneral.getRequest().setNOTES1(this.toAscii(this.getNOTES1())); //鈔箱1狀態
		atmGeneral.getRequest().setNOTES2(this.toAscii(this.getNOTES2())); //鈔箱2狀態
		atmGeneral.getRequest().setNOTES3(this.toAscii(this.getNOTES3())); //鈔箱3狀態
		atmGeneral.getRequest().setNOTES4(this.toAscii(this.getNOTES4())); //鈔箱4狀態
		atmGeneral.getRequest().setJOUS(this.toAscii(this.getJOUS())); //序時紙捲狀態
		atmGeneral.getRequest().setADVS(this.toAscii(this.getADVS())); //交易明細表狀態
		atmGeneral.getRequest().setDEPS(this.toAscii(this.getDEPS())); //存款模組狀態
		atmGeneral.getRequest().setMCRWS(this.toAscii(this.getMCRWS())); //磁條讀寫頭狀態
		atmGeneral.getRequest().setENCRS(this.toAscii(this.getENCRS())); //ENCRYPTOR狀態
		atmGeneral.getRequest().setSTAMS(this.toAscii(this.getSTAMS())); //對帳單模組狀態
		atmGeneral.getRequest().setDISPEN(this.toAscii(this.getDISPEN())); //吐鈔模組狀態
		atmGeneral.getRequest().setFILLER1(this.toAscii(this.getFILLER1())); //FILLER1
		atmGeneral.getRequest().setSERVICE(this.toAscii(this.getSERVICE())); //ATM 服務記號
		atmGeneral.getRequest().setMODE(this.toAscii(this.getMODE())); //營業日MODE
		atmGeneral.getRequest().setDD(this.toAscii(this.getDD())); //入帳日
		atmGeneral.getRequest().setDEPMODE(this.toAscii(this.getDEPMODE())); //信封存款MODE
		atmGeneral.getRequest().setAtmseq_1(this.toAscii(this.getAtmseq_1())); //ATM 系統日
		atmGeneral.getRequest().setAtmseq_2(this.toAscii(this.getAtmseq_2())); //ATM交易序號
		atmGeneral.getRequest().setBKNO(this.toAscii(this.getBKNO())); //銀行別
		atmGeneral.getRequest().setCHACT(this.toAscii(this.getCHACT())); //卡片帳號
		atmGeneral.getRequest().setTXACT(this.toAscii(this.getTXACT())); //交易帳號
		atmGeneral.getRequest().setBknoD(this.toAscii(this.getBknoD())); //轉入銀行別
		atmGeneral.getRequest().setActD(this.toAscii(this.getActD())); //轉入帳號
		atmGeneral.getRequest().setFPB(this.toAscii(this.getFPB())); //FOR MATED PIN BLOCK
		atmGeneral.getRequest().setTRACK3(this.toAscii(this.getTRACK3())); //第三軌資料
		atmGeneral.getRequest().setTXAMT(this.toBigDecimal(this.getTXAMT()));
		atmGeneral.getRequest().setDSPCNT1(this.toBigDecimal(this.getDSPCNT1()));
		atmGeneral.getRequest().setDSPCNT2(this.toBigDecimal(this.getDSPCNT2()));
		atmGeneral.getRequest().setDSPCNT3(this.toBigDecimal(this.getDSPCNT3()));
		atmGeneral.getRequest().setDSPCNT4(this.toBigDecimal(this.getDSPCNT4()));
		atmGeneral.getRequest().setCLASS(this.toAscii(this.getCLASS())); //繳款種類
		atmGeneral.getRequest().setPAYCNO(this.toAscii(this.getPAYCNO())); //銷帳編號
		atmGeneral.getRequest().setDUEDATE(this.toAscii(this.getDUEDATE())); //繳款到期日
		atmGeneral.getRequest().setUNIT(this.toAscii(this.getUNIT())); //稽徵機關別
		atmGeneral.getRequest().setIDNO(this.toAscii(this.getIDNO())); //身分證字號
		atmGeneral.getRequest().setAPPLY(this.toAscii(this.getAPPLY())); //APPLY
		atmGeneral.getRequest().setDATE(this.toAscii(this.getDATE())); //預約轉帳入賬日期
		atmGeneral.getRequest().setATMCHK(this.toAscii(this.getATMCHK())); //端末設備查核碼
		atmGeneral.getRequest().setICDTTM(this.toAscii(this.getICDTTM())); //YYYYMMDDHHMMSS
		atmGeneral.getRequest().setICACT(this.toAscii(this.getICACT())); //晶片主帳號
		atmGeneral.getRequest().setICMARK(this.getICMARK()); //晶片卡REMARK欄位資料
		atmGeneral.getRequest().setICTXSEQ(this.toAscii(this.getICTXSEQ())); //晶片卡交易序號
		atmGeneral.getRequest().setICTAC(this.toAscii(this.getICTAC())); //晶片卡交易驗證碼
		atmGeneral.getRequest().setOriTxseq(this.toAscii(this.getOriTxseq())); //原REQ訊息中心交易序號
		atmGeneral.getRequest().setOriAtmseq_1(this.toAscii(this.getOriAtmseq_1())); //原REQ訊息ATM系統日
		atmGeneral.getRequest().setOriAtmseq_2(this.toAscii(this.getOriAtmseq_2())); //原REQ訊息ATM交易序號
		...
		atmGeneral.getRequest().setYYMMDD(this.toAscii(this.getYYMMDD())); //壓碼日期
		atmGeneral.getRequest().setMAC(this.toAscii(this.getMAC())); //訊息押碼
		atmGeneral.getRequest().setCURCD(this.toAscii(this.getCURCD())); //提領幣別
		atmGeneral.getRequest().setACTCNT(this.toAscii(this.getACTCNT())); //調帳號記號
		atmGeneral.getRequest().setRATE(this.toBigDecimal(this.getRATE(), 5));
		atmGeneral.getRequest().setNTAMT(this.toBigDecimal(this.getNTAMT()));
		atmGeneral.getRequest().setACRAT(this.toBigDecimal(this.getACRAT(), 5));
		atmGeneral.getRequest().setDISRAT(this.toBigDecimal(this.getDISRAT(), 5));
		atmGeneral.getRequest().setCHARGE(this.toBigDecimal(this.getCHARGE(), 2));
		atmGeneral.getRequest().setEXP(this.toBigDecimal(this.getEXP(), 2));
		atmGeneral.getRequest().setSCASH(this.toBigDecimal(this.getSCASH(), 5));
		atmGeneral.getRequest().setDepType(this.toAscii(this.getDepType())); //信封存款種類
		atmGeneral.getRequest().setDepDepno(this.toAscii(this.getDepDepno())); //存款信封序號
		atmGeneral.getRequest().setDSPCNT1T(this.toBigDecimal(this.getDSPCNT1T()));
		atmGeneral.getRequest().setDSPCNT2T(this.toBigDecimal(this.getDSPCNT2T()));
		atmGeneral.getRequest().setDSPCNT3T(this.toBigDecimal(this.getDSPCNT3T()));
		atmGeneral.getRequest().setDSPCNT4T(this.toBigDecimal(this.getDSPCNT4T()));
		atmGeneral.getRequest().setVPID(this.toAscii(this.getVPID())); //委託單位代號
		atmGeneral.getRequest().setPAYID(this.toAscii(this.getPAYID())); //費用代號
		atmGeneral.getRequest().setMENO(this.toAscii(this.getMENO())); //附言欄
		atmGeneral.getRequest().setDSPCNT5(this.toBigDecimal(this.getDSPCNT5()));
		atmGeneral.getRequest().setDSPCNT6(this.toBigDecimal(this.getDSPCNT6()));
		atmGeneral.getRequest().setDSPCNT7(this.toBigDecimal(this.getDSPCNT7()));
		atmGeneral.getRequest().setDSPCNT8(this.toBigDecimal(this.getDSPCNT8()));
		atmGeneral.getRequest().setTXAMTDB(this.toBigDecimal(this.getTXAMTDB(), 2)); //折合扣帳幣別金額
		atmGeneral.getRequest().setBalCur(this.toAscii(this.getBalCur())); //扣帳幣別，海外卡專用
		atmGeneral.getRequest().setHcCur(this.toAscii(this.getHcCur())); //手續費幣別，搭配EXP
		atmGeneral.getRequest().setPHONENO(this.toAscii(this.getPHONENO())); //聯絡電話
		atmGeneral.getRequest().setIPADDR(this.toAscii(this.getIPADDR())); //使用者登入IP
		atmGeneral.getRequest().setFILLER2(this.toAscii(this.getFILLER2())); //FILLER2
		atmGeneral.getRequest().setNOTES5(this.toAscii(this.getNOTES5())); //鈔箱5狀態
		atmGeneral.getRequest().setNOTES6(this.toAscii(this.getNOTES6())); //鈔箱6狀態
		atmGeneral.getRequest().setNOTES7(this.toAscii(this.getNOTES7())); //鈔箱7狀態
		atmGeneral.getRequest().setNOTES8(this.toAscii(this.getNOTES8())); //鈔箱8狀態
		atmGeneral.getRequest().setFILLER3(this.toAscii(this.getFILLER3())); //FILLER3
		*/
	}
}
