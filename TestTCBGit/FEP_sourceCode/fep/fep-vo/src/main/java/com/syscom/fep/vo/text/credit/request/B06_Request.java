...
public class B06_Request extends CreditTextBase {
    //交易序號
    @Field(length = 6)
    ...
    //
    @Field(length = 527, optional = true)
    ...
    public void setBSP(String value) {
        _BSP = value;
    }
    ...
    public String makeMessageFromGeneral(CreditGeneral general) throws Exception {
        ...
        _ASP = StringUtils.rightPad(tempVar.getASP(),1,' '); //信用卡主機狀態
        _BSP = StringUtils.rightPad(tempVar.getBSP(),1,' '); //銀行主機狀態
        _FILLER1 = tempVar.getFILLER1();
        return _NUMBER + _CARD_NO + _ACCOUNT + _CD1 + _CD2 + _CD3 + _TXDATE + _KINBR + _WSNO + _TXSEQ + _TBSDY + _MODE + _TXCD + _RC + _ASPSTAN + _CARDNO + _ACTNO + _SEQNO + _ASP + _BSP + _FILLER1;
