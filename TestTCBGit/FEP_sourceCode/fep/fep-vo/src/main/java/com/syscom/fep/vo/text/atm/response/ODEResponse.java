...
public class ODEResponse extends ATMTextBase {
	// 交易類別
	@Field(length = 6)
	...
	// 收取手續費金額
	@Field(length = 8, optional = true)
	private String _ACFEE = StringUtils.EMPTY;

	//
	@Field(length = 1370, optional = true)
	...
	public String makeMessageFromGeneral(ATMGeneral general) throws Exception {
		...
		_TXCD_O = StringUtil.toHex(StringUtils.rightPad(response.getTxcdO(), 3, StringUtils.SPACE)); // 交易類別
		_DATE = StringUtil.toHex(StringUtils.leftPad(response.getDATE(), 8, '0')); // 交易日期
		_TIME = StringUtil.toHex(StringUtils.leftPad(response.getTIME(), 6, '0')); // 交易時間
		_ATMNO_O = StringUtil.toHex(StringUtils.leftPad(response.getAtmnoO(), 5, '0')); // ATM ID
		_MODE_O = StringUtil.toHex(StringUtils.rightPad(response.getModeO(), 1, StringUtils.SPACE)); // MODE
		_DD_O = StringUtil.toHex(StringUtils.leftPad(response.getDdO(), 8, '0')); // 本營業日
		_DEPMODE_O = StringUtil.toHex(StringUtils.rightPad(response.getDepmodeO(), 1, StringUtils.SPACE)); // 信封存款MODE
		_ATMSEQ_O1 = StringUtil.toHex(StringUtils.leftPad(response.getAtmseqO1(), 8, '0')); // ATM序號1
		_ATMSEQ_O2 = StringUtil.toHex(StringUtils.leftPad(response.getAtmseqO2(), 8, '0')); // ATM序號2
		_REJCD = StringUtil.toHex(StringUtils.rightPad(response.getREJCD(), 4, StringUtils.SPACE)); // 拒絕理由
		_TXSEQ = StringUtil.toHex(StringUtils.leftPad(response.getTXSEQ(), 7, '0')); // 中心交易序號
		_ACFEE = this.toHex(response.getACFEE(), 4, 0); // 收取手續費金額
		_FILLER1 = StringUtil.toHex(StringUtils.leftPad(StringUtils.EMPTY, 685, StringUtils.SPACE));

		return StringUtils.join(_TXCD_O, this._DATE, this._TIME, this._ATMNO_O, this._MODE_O, this._DD_O, this._DEPMODE_O, this._ATMSEQ_O1, this._ATMSEQ_O2, this._REJCD, this._TXSEQ, this._ACFEE,
				this._FILLER1);
		*/return "";
    }
