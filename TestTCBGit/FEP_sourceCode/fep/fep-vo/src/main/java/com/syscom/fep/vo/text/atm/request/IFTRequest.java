...
public class IFTRequest extends ATMTextBase {
	// 空白
	@Field(length = 4)
	private String _TA = "";
	...
	// 晶片卡REMARK欄位資料
	@Field(length = 60, optional = true, toAscii = false)
	...
	// 附言欄
	@Field(length = 80, optional = true, toAscii = false)
	...
	// 中文備註
	@Field(length = 56, optional = true, toAscii = false)
	private String _CHREM = "";
	...
	public void toGeneral(ATMGeneral general) throws Exception {
		...
		general.getRequest().setMENO(_MENO); // 附言欄
		...
		general.getRequest().setIPADDR(this.toAscii(_IPADDR)); // 使用者登入IP
		general.getRequest().setCHREM(_CHREM); // 中文備註
