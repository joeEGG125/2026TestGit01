...
public class B07_Request extends CreditTextBase {
    ...
    //
    @Field(length = 211, optional = true)
    ...
    public void setPOSENTRY(String value) {
        _POSENTRY = value;
    }
    ...
    public String makeMessageFromGeneral(CreditGeneral general) throws Exception {
        ...
        _CLASS = StringUtils.leftPad(tempVar.getCLASS(),5, '0'); //繳款類別
        _NOTICENO = StringUtils.leftPad(tempVar.getNOTICENO(),16, '0'); //銷帳編號
        ...
        _ICMARK = StringUtils.rightPad(tempVar.getICMARK(),30, ' '); //IMC Mark
        _TripleDES = StringUtils.rightPad(tempVar.getTripleDES(),1, ' '); //PINBLOCK押碼方式
        _POSENTRY = StringUtils.rightPad(tempVar.getPOSENTRY(),4, ' '); //POS ENTRY MODE
        _FILLER1 = tempVar.getFILLER1();

        return _NUMBER + _CARD_NO + _ACCOUNT + _CD1 + _CD2 + _CD3 + _TXDATE + _KINBR + _WSNO + _TXSEQ + _TBSDY + _MODE + _TXCD + _RC + _ASPSTAN + _CARDNO + _ACTNO + _SEQNO + _APID + _BANK_W + _ACTNO_W + _BANK_D + _ACTNO_D + _TXAMT + _FEE + _AUTHCD + _ID + _FPB + _TRACK3 + _AVBAL_S + _AVBAL + _ACTBAL_S + _ACTBAL + _CLASS + _NOTICENO + _ATMID + _STAN + _MEMNO + _MAC + _ICMARK + _TripleDES + _POSENTRY + _FILLER1;
    }
