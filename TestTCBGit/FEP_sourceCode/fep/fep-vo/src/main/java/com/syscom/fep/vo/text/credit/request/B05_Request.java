...
public class B05_Request extends CreditTextBase {
    //交易序號
    @Field(length = 6)
    ...
    //
    @Field(length = 467, optional = true)
    ...
    public String makeMessageFromGeneral(CreditGeneral general) throws Exception {
        ...
        _TRACK_II = StringUtils.rightPad(tempVar.getTrackIi(),37,' '); //磁條二軌資料
        ...
        return _NUMBER + _CARD_NO + _ACCOUNT + _CD1 + _CD2 + _CD3 + _TXDATE + _KINBR + _WSNO + _TXSEQ + _TBSDY + _MODE + _TXCD + _RC + _ASPSTAN + _CARDNO + _ACTNO + _SEQNO + _TRACK_II + _FPB + _MAC + _TripleDES + _FILLER1;
    }
