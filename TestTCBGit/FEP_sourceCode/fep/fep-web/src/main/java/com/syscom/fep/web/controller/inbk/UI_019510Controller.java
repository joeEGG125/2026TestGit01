...
import com.syscom.fep.web.form.inbk.UI_019510_Form;
 ...
 */
@Controller
public class UI_019510Controller extends BaseController {
    @Autowired
    InbkService inbkService;

    @Override
    public void pageOnLoad(ModelMap mode) {
        UI_019510_Form form = new UI_019510_Form();
        // UI_營業日預設營業日
        form.setFeptxnTbsdyFisc(FormatUtil.dateTimeFormat(Calendar.getInstance(), FormatUtil.FORMAT_DATE_YYYY_MM_DD_DASH));
        WebUtil.putInAttribute(mode, AttributeName.Form, form);
    }

    @PostMapping(value = "/inbk/UI_019510/queryClick", produces = "application/json;charset=utf-8")
    public String queryClick(@ModelAttribute UI_019510_Form form, ModelMap mode) {
        bindData(form, mode);
        return Router.UI_019510.getView();
    }

    protected void bindData(UI_019510_Form form, ModelMap mode) {
        ...
        try {
            ...
            //查詢 (收款/原存)
            PageInfo<HashMap<String, Object>> dt = inbkService.query2700TxnResult(queryDate, brapSelect, form.getPageNum(), size, "019510");

            if (dt == null || dt.getList().size() == 0 || dt.getList().get(0) == null) {
                this.showMessage(mode, MessageType.INFO, QueryNoData);
                PageData<UI_019510_Form, HashMap<String, Object>> pageData = new PageData<>(dt, form);
