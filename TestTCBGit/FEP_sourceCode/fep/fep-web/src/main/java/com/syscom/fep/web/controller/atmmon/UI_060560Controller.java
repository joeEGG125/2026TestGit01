...
import com.syscom.fep.web.form.atmmon.UI_060550_Ucdid_Form;
import com.syscom.fep.web.form.atmmon.UI_060560_FormDetail;
import com.syscom.fep.web.form.atmmon.UI_060560_FormMain;
...
/**
 * 歷史交易日誌(FEPTXN)查詢
 * 2026.1.19 同步 UI_060550 功能
 *
 * @author
 */
@Controller
public class UI_060560Controller extends BaseController {
    @Autowired
    ...
    public void pageOnLoad(ModelMap mode) {
        // 初始化表單資料
        UI_060560_FormMain form = new UI_060560_FormMain();
        form.setFeptxnTbsdyFisc(this.getTbsdy(mode));
        // 060560 預設排除查詢PCODE，060550為 "5202;3113"
        ...
        // 取得當前使用者群組 (2026.1.19同步 060550 邏輯)
    ...
    private String getTbsdy(ModelMap mode) {
        try {
            // 2026.1.19 同步 060550 改用 sysstatExtMapper
    ...
    /**
     * 通道下拉選單 (2026.1.19 同步 060550)
     *
     * @param mode
     * @throws Exception
     */
    private void setChannelOptions(ModelMap mode) throws Exception {
        List<Channel> channelList = channelService.queryAllDataAsc();
    ...
    @PostMapping(value = "/atmmon/UI_060560/inquiryMain")
    public String doInquiryMain(@ModelAttribute UI_060560_FormMain form, ModelMap mode) {
        ...
        try {
            if (this.doValidateForm(form, mode)) {
                ...
                // 維持呼叫 getHisFeptxn (歷史)
                PageInfo<Feptxn> pageInfo = atmService.getHisFeptxn(argsMap);
                if (pageInfo.getSize() == 0) {
                    this.showMessage(mode, MessageType.INFO, QueryNoData);
                }
                PageData<UI_060560_FormMain, Feptxn> pageData = new PageData<UI_060560_FormMain, Feptxn>(pageInfo, form);
                WebUtil.putInAttribute(mode, AttributeName.PageData, pageData);
                mode.addAttribute("totalCount", pageInfo.getTotal());
                // 查詢總金額
                Map<String, Object> summary = atmService.getHisFeptxnSummary(argsMap);
        ...
        return Router.UI_060560.getView();
    }
     ...
     * @return
     */
    private boolean doValidateForm(UI_060560_FormMain form, ModelMap mode) {
        int cnt = 0;
        if (StringUtils.isNotBlank(form.getFeptxnEjfno())) {
    ...
    @PostMapping(value = "/atmmon/UI_060560/inquiryDetail")
    public String doInquiryDetail(@ModelAttribute UI_060560_FormDetail form, ModelMap mode) {
     ...
     * @return
     */
    private String doInquiryDetailForOtherSubSys(UI_060560_FormDetail form, ModelMap mode) {
        try {
            ...
            // 維持呼叫 getHisFeptxnIntltxn (歷史)
            Map<String, Object> detail = atmService.getHisFeptxnIntltxn(tableNameSuffix, form.getFeptxnEjfno(), form.getFeptxnTxDate());
            // 2010-10-14 by kyo for 跨區交易時營業日會使用卡片地區營業日
            if (MapUtils.isEmpty(detail)) {
                // 取出查詢主頁資料時候的表單資料
                UI_060560_FormMain formMain = (UI_060560_FormMain) WebUtil.getUser().getPrevPageForm();
                ...
                detail = atmService.getHisFeptxnIntltxn(tableNameSuffix, form.getFeptxnEjfno(), form.getFeptxnTxDate());
            }
            if (MapUtils.isEmpty(detail)) {
                this.showMessage(mode, MessageType.INFO, QueryNoData);
            }
            ...
            if (StringUtils.equals(feptxnCbsProc, "N")) {
                // 維持呼叫 getHisFeptxntcb (歷史)
                feptxntcb = atmService.getHisFeptxntcb(form.getFeptxnTxDate(), form.getFeptxnEjfno());
            }
            ...
            if (MapUtils.isNotEmpty(detail)) {
                ...
                if (txCode != null && ((txCode.startsWith("LF") && "TT".equals(activityCode)) || txCode.startsWith("WP"))) {
                    ...
                    // 維持呼叫 getHisUcdidDataByLogic (歷史)
                    Map<String, Object> ucdidData = atmService.getHisUcdidDataByLogic(idno, healthId, txCode, txDate, stan);
            ...
            // 060550 使用 Integer ejfno, 060560 使用 string stan, 保留560查詢條件
                String feptxnStan = (String) detail.getOrDefault("FEPTXN_STAN", null);
                inbk2160 = hceService.getHisINBK2160ByFeptxn(feptxnTxDate, feptxnStan);
            // }
            form.setInbk2160(inbk2160);

            // 2026.1.19 同步 060550，處理匯率顯示
        ...
        return Router.UI_060560_Detail.getView();
    }

    @PostMapping(value = "/atmmon/UI_060560/ucdidHistory")
    public String doUcdidHistory(UI_060550_Ucdid_Form form, ModelMap mode) {
        this.changeTitle(mode, Router.UI_060560_Ucdid.getName());
        this.infoMessage("查詢明細資料, 條件 = [", form.toString(), "]");
        this.doKeepFormData(mode, form);
        try {
...
//                return Router.UI_060550_Detail.getView();
        ...
        return Router.UI_060560_Ucdid.getView();
    }

    /**
     * 下載 FEPTXN 交易明細 (2026.1.19 同步 060550)
     * 排除 Log 下載，且使用 History Service
     */
    @PostMapping(value = "/atmmon/UI_060560/downloadFeptxn")
    @ResponseBody
    public ResponseEntity<StreamingResponseBody> downloadFeptxn(@ModelAttribute UI_060560_FormMain form, ModelMap mode) {
        this.infoMessage("開始下載檔案, 條件 = [", form.toString(), "]");
        try {
            UI_060560_FormMain queryForm = (UI_060560_FormMain) WebUtil.getUser().getCurrentPageForm();
            if (queryForm == null) {
            ...
            String fileName = StringUtils.join("HisFeptxn_", new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()), ".csv");
    ...
    /**
     * 準備查詢參數 Map (2026.1.19 同步 060550 邏輯)
     */
    private Map<String, Object> prepareArgsMap(UI_060560_FormMain form) {
        Map<String, Object> argsMap = form.toMap();
        ...
        // History 表尾邏輯 (同步 060550 判斷)
        if (StringUtils.isNotBlank(feptxnTbsdyFisc) && feptxnTbsdyFisc.length() >= 8) {
            argsMap.put("tableNameSuffix", feptxnTbsdyFisc.substring(6, 8));
        } else {
            argsMap.put("tableNameSuffix", "00"); // 安全預設
        }
