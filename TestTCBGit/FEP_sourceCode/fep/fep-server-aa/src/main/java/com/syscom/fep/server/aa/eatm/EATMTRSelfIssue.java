 ...
 * @author Jaime
 */
public class EATMTRSelfIssue extends ATMPAABase {
	private boolean goTO7 = true;
	private Object tota = null;
	private FEPReturnCode rtnCode = FEPReturnCode.Normal;

	public EATMTRSelfIssue(ATMData txnData) throws Exception {
		super(txnData,"eatm");
	}
	...
	private void sendToCBS() throws Exception {
		...
		if("T".equals(feptxn.getFeptxnMsgid()) || "X".equals(feptxn.getFeptxnMsgid())){
			feptxn.setFeptxnStan(this.getTxData().getTxObject().getEatmrequest().getBody().getRq().getSvcRq().getPIEOSTAN());
		}
		/* 交易記帳處理 */
	...
	private String response() throws Exception {
		...
		try {
			...
			if (rtnCode != FEPReturnCode.Normal) {
				...
				// 格式:$999 ex :$0
				msgbody.setPFEE(StringUtils.leftPad(FormatUtil.decimalFormat(feptxn.getFeptxnFeeCustpayAct(), "$#,##0"), 4, " "));
