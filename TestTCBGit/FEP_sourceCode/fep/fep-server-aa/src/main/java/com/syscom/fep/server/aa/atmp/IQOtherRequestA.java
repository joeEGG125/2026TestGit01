	...
	public String processRequestData() throws Exception {
		...
		try {
			...
			if (rtnCode == FEPReturnCode.Normal) {
        		...
        		if (rtnCode != FEPReturnCode.Normal) {
					fepfail = 1;
				}else if(!StringUtils.equals(feptxn.getFeptxnRepRc(), "4001") ) {
					fiscfail = 1;
				}
			}
			...
			if (atmrepfpc == 1) {
				...
				rtnMessage = atm_faa_cc1apc.makeMessage();
				 ...
				 if(!"ATM".equals(feptxn.getFeptxnChannel())) {
	                	rtnMessage = atm_faa_cc1apc.getMSGCAT() + atm_faa_cc1apc.getMSGTYP()
							+ atm_faa_cc1apc.getTRANDATE() + atm_faa_cc1apc.getTRANTIME() 
							+ feptxn.getFeptxnStan()
	                		+ feptxn.getFeptxnAtmno() +  atmReq.getTRANSEQ()
	                		+ atmReq.getTDRSEG() + feptxn.getFeptxnReplyCode()
	                		;
				...
				rtnMessage = atm_faa_cc1apn.makeMessage();
				 ...
				 if(!"ATM".equals(feptxn.getFeptxnChannel())) {
	                	rtnMessage = atm_faa_cc1apn.getMSGCAT() + atm_faa_cc1apn.getMSGTYP()
							+ atm_faa_cc1apn.getTRANDATE() + atm_faa_cc1apn.getTRANTIME() 
							+ feptxn.getFeptxnStan()
	                		+ feptxn.getFeptxnAtmno() +  atmReq.getTRANSEQ()
	                		+ atmReq.getTDRSEG() + feptxn.getFeptxnReplyCode()
	                		;
	...
	private void updateTxData() {
		...
		if (fepfail == 0 && fiscfail == 0 && rtnCode == FEPReturnCode.Normal) {
			...
			if (feptxn.getFeptxnFiscTimeout() != null ) {
				// 查詢交易回覆前端逾時
				feptxn.setFeptxnTxrust("R"); /* Reject-abnormal */
