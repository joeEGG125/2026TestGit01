 ...
 * @author
 */
public class EATMTRSelfRequestA extends INBKAABase {
    ...
    public EATMTRSelfRequestA(ATMData txnData) throws Exception {
        super(txnData, "eatm");
    }
    ...
    private FEPReturnCode checkBusinessRule() {
        ...
        try {
            ...
            if (rtnCode != FEPReturnCode.Normal) {
                return rtnCode; // GOTO label_END_OF_FUNC /* 組回傳 ATM 電文 */
            }

            // 檢核財金及參加單位之系統狀態
            rtnCode = getATMBusiness().checkINBKStatus(getFeptxn().getFeptxnPcode(), true);
            if (rtnCode != FEPReturnCode.Normal) {
                return rtnCode;
            }
    ...
    private String response() throws Exception {
        ...
        try {
            ...
            if (rtnCode != CommonReturnCode.Normal || !NormalRC.FISC_ATM_OK.equals(getFeptxn().getFeptxnRepRc())) {
                ...
                if (StringUtils.isNotBlank(feptxn.getFeptxnCbsRc()) && !feptxn.getFeptxnCbsRc().equals("000")
                        && !feptxn.getFeptxnCbsRc().equals("4001") && !feptxn.getFeptxnCbsRc().equals("0000")) {
                ...
                if (StringUtils.isNotBlank(totaToact)) {
                    ...
                    msgbody.setPTRINACCT(feptxn.getFeptxnTrinBkno());
                }
                ...
                SEND_EATM_FAA_CC1B1PN_Body_MsgRs_SvcRs msgbody = new SEND_EATM_FAA_CC1B1PN_Body_MsgRs_SvcRs();
                msgrs.setHeader(header);
                msgrs.setSvcRq(msgbody);
                rsbody.setRs(msgrs);
                rs.setBody(rsbody);
