...
public class PendingAskFromBank2290 extends INBKAABase {
    ...
    private List<Feptxn> oriFeptxn = new ArrayList<>();
     ...
     * 讀取交易記錄檔FEPTXN
     *
     * @return <modify> <modifier>HusanYin</modifier> <reason>修正Const RC</reason>
     * <date>2010/11/25</date> </modify>
     */
    private FEPReturnCode readFeptxn() {
        FEPReturnCode rtnCode = CommonReturnCode.Normal;
        String wk_LLBSDY_FISC = null;
        ...
        try {
            ...
            if (bsdays == null) {
                getLogContext().setRemark("查無財金前前前營業");
                sendEMS(getLogContext());
                return FEPReturnCode.ProgramException;
            }
...
//				feptxnDao.setTableNameSuffix(tbsdy, StringUtils.join(ProgramName, ".processRequestData"));
                ...
                /* 2025/9/10 修改 for  2280 判斷財金營業日 */
                oriFeptxn = feptxnDao.getFEPTXNFor2290(wk_TXDATE_S, wk_TXDATE_E, getFiscBusiness().getFeptxn().getFeptxnTxDate(), getFiscBusiness().getFeptxn().getFeptxnBkno(),
     ...
     * 將 FEPTXN 逐筆讀出並送至財金
     *
     * @return <modifier>HusanYin</modifier> <reason>修正Const RC</reason>
     * <date>2010/11/25</date> </modify>
     */
    private FEPReturnCode readDataAndSendToFISC() {
        ...
        try {
            if (String.valueOf(FISCPCode.PCode2290.getValueStr()).equals(uiPCODE)) {
                FeptxnCount = oriFeptxn.size();
            }

            for (i = 0; i < FeptxnCount; i++) {
                /* 2026/04/01 單筆2290開始前初始化 */
                defINBKPEND = new Inbkpend();
                desFEPTXN = new FeptxnExt();
                getFiscRes().setRsCode("");

                if (FISCPCode.PCode2290.getValueStr().equals(uiPCODE)) {
                    getFiscBusiness().setFeptxn(new FeptxnExt(oriFeptxn.get(i)));
                ...
                if (rtnCode != CommonReturnCode.Normal) {
                    ...
                    if ("2290".equals(getFiscRes().getProcessingCode())) {
                        getTxData().getTxObject().setINBKRequest(getFiscRes());
                        defINBKPEND.setInbkpendPrcResult(getFiscRes().getRsCode());
                        if (StringUtils.isNotBlank(getFiscRes().getAcctSup())) {
                            defINBKPEND.setInbkpendAcctSup(getFiscRes().getAcctSup());/* 帳戶補充資訊*/
                        }
                    }
                }
                ...
                // (7) 檢核 ORI_STAN
                if (!getFiscRes().getOriStan().equals(getFiscReq().getOriStan())) {
                ...
                // (11) @SendToCBS: 帳務主機處理
                rtnCode = sendToCBS(oriFeptxn.get(i));
                if (rtnCode != CommonReturnCode.Normal) {
                    return rtnCode;
                }
            }
    ...
    private FEPReturnCode prepareINBKPEND(int drCount) {
        try {
            ...
            if (drCount > 0) {
                ...
                getTxData().setEj(defINBKPEND.getInbkpendEjfno());
            } else {
                // 單筆或多筆的第一筆
...
//        getFiscBusiness().getFeptxn().setFeptxnConRc(defINBKPEND.getInbkpendPrcResult()); /* 處理結果 */
     ...
     * @throws Exception
     */
    private FEPReturnCode sendToCBS(Feptxn oriFeptxn) {
        FEPReturnCode rc1 = CommonReturnCode.Normal;
        String TxType;
        try {
            if (NormalRC.FISC_OK.equals(getFiscRes().getResponseCode())) {
                TxType = "0"; //查詢

                String AA = getTxData().getMsgCtl().getMsgctlTwcbstxid();
                ACBSAction hostAA = (ACBSAction) this.getInstanceObject(AA, getTxData());
                rc1 = new CBS(hostAA, getTxData()).sendToCBS(TxType);
                /* 2026/3/17 修改為 判斷FEPTXN_ CBS_TIMEOUT */
                if( getFiscBusiness().getFeptxn().getFeptxnCbsTimeout() == 0){
                    //FEP依主機回覆記帳狀況,更新原交易結果(記帳或沖正),並更新為交易完成(解除PENDING)
                    /* 2024/10/23 修改為 判斷FEPTXN_CBS_RC */
                    if(NormalRC.FISC_ATM_OK.equals(getFiscBusiness().getFeptxn().getFeptxnCbsRc())){
                        oriFeptxn.setFeptxnAccType(getFiscBusiness().getFeptxn().getFeptxnAccType());
                        oriFeptxn.setFeptxnPending((short) 2);  //解除 PENDING
                        feptxnDao.updateByPrimaryKey(oriFeptxn);
                        if (feptxnDao.updateByPrimaryKey(oriFeptxn) < 1) {
                            return IOReturnCode.UpdateFail;
                        }
                    }
                }
