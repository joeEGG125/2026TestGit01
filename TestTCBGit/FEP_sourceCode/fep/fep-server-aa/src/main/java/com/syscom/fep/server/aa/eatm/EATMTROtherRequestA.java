 ...
 * @author Jaime
 */
public class EATMTROtherRequestA extends INBKAABase {
	...
	public EATMTROtherRequestA(ATMData txnData) throws Exception {
		super(txnData,"eatm");
	}
		...
		try {
			...
			if (rtnCode != CommonReturnCode.Normal || !NormalRC.FISC_ATM_OK.equals(getFeptxn().getFeptxnRepRc())) {
				...
				if(StringUtils.isNotBlank(feptxn.getFeptxnBalb().toString())){
					FormatUtil.decimalFormat(feptxn.getFeptxnBalb(), "$#,##0.00");
				}
				msgbody.setPBAL(StringUtils.leftPad(PBAL, 18, " "));
				// CBS下送的轉出帳號(明細表顯示內容)
