 ...
 * @author
 */
public class NBTROtherRequestA extends INBKAABase {
    ...
    public NBTROtherRequestA(NBData txnData) throws Exception {
        super(txnData);
    }
    ...
    public String processRequestData() throws Exception {
        ...
        try {
            ...
            if(StringUtils.equals(getLogContext().getChannel().getNameS(),FEPChannel.VO.getNameS())){
                rtnMessage = getFiscBusiness().prepareVOResponseData(tota1);
            } else {
                rtnMessage = getFiscBusiness().prepareNBTRResponseData(tota1, getLogContext().getChannel());
            }
            ...
            if (StringUtils.isNotBlank(feptxn.getFeptxnRepRc()) && !"4001".equals(feptxn.getFeptxnRepRc()) &&
                    (("2521".equals(feptxn.getFeptxnPcode()) && "T4".equals(feptxn.getFeptxnTxCode()))
                            || "2522".equals(feptxn.getFeptxnPcode()))) { // 財金回覆失敗再送
                AATxTYPE = "";
                AATxRs = "N";
                String AA = "CBTXEND";
            ...
            if ("TS".equals(StringUtils.left(this.feptxn.getFeptxnTxCode(), 2)) && "2521".equals(this.feptxn.getFeptxnPcode())) {
                ...
                if (rtnCode4 != FEPReturnCode.Normal) {
                    ...
                    if (feptxn.getFeptxnCbsTimeout() == 0) {
                        ...
                        feptxn.setFeptxnReplyCode(TxHelper.getRCFromErrorCode(Objects.toString(rtnCode4.getValue()),
                                FEPChannel.FEP, FEPChannel.ATM, getnBData().getLogContext()));
                        getLogContext().setReturnCode(rtnCode4);
                        getLogContext().setRemark("sendToCBS Error");
