 ...
 * @author Jaime
 */
public class EATMPYOtherRequestA extends INBKAABase {
	private Object tota = null;
	private FEPReturnCode rtnCode = FEPReturnCode.Normal;
	private FEPReturnCode rtnCode2 = FEPReturnCode.Normal;

	public EATMPYOtherRequestA(ATMData txnData) throws Exception {
		super(txnData,"eatm");
	}
	...
	private void sendToCBS() throws Exception {
		if (feptxn.getFeptxnTrinBkno().equals(SysStatus.getPropertyValue().getSysstatHbkno()) ||
				!("18888888".equals(feptxn.getFeptxnBusinessUnit()) && "59999".equals(feptxn.getFeptxnPaytype())
						&& "9999".equals(feptxn.getFeptxnPayno()))) {
			// 轉入方為本行時,先送CBS查詢帳號
	...
	private void labelEndOfFunc() {
		...
		if ((rtnCode == FEPReturnCode.Normal) && "4001".equals(feptxnRepRc)) {
			...
			feptxn.setFeptxnMsgflow("A2"); /* ATM Response */
			this.updateFeptxn(); /* 檔名SEQ為 FEPTXN_TBSDY_FISC[7:2] */
			// 組回應電文回給 ATM, 寫入 ATM Response Queue // TODO
		}
	}
