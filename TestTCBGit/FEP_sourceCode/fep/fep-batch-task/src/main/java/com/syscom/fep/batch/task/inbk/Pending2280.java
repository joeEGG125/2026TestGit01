...
public class Pending2280 extends FEPBase implements Task {
	...
	public BatchReturnCode execute(String[] args) {
		try {
			...
			// 3. 批次主要處理流程
			// 檢查有值再執行呼叫 AA
			if(checkTbsdyFisc() && check2280Data()) {
	...
	private boolean mainProcess() {
		...
		try {
			...
			job.writeLog("Call PendingFromBank送2280完成");
...
//            }
	...
	private boolean check2280Data() {
		...
		try {
			...
			sysDateTime.add(Calendar.MINUTE, -10); /* 2025/9/5 修改 for  2280 檢核原交易超過  10 分鐘才發送 */
			
			String systemDateTime = new SimpleDateFormat(FormatUtil.FORMAT_DATE_YYYYMMDDHHMMSS_PLAIN).format(sysDateTime.getTime());
			String sysstatHbkno = _defSYSSTAT.get(0).get("SYSSTAT_HBKNO").toString();
//			String sysstatTbsdy = _defSYSSTAT.get(0).get("SYSSTAT_TBSDY_FISC").toString();
//			String sysstatLbsdy = _defSYSSTAT.get(0).get("SYSSTAT_LBSDY_FISC").toString();
			job.writeLog(ProgramName + " - SYSETM_DATETIME(10 minutes ago):" + systemDateTime + ";");
			job.writeLog(ProgramName + " - SYSSTAT_TBSDY_FISC:" + wk_DATE + ";");
			job.writeLog(ProgramName + " - SYSSTAT_HBKNO:" + sysstatHbkno + ";");
			
			List <Map <String, Object>> feptxnCountsList = feptxnExtMapper.getFEPTXN2280Count(wk_TXDATE_S, wk_TXDATE_E, systemDateTime, sysstatHbkno, wk_DATE);
	...
	private boolean checkTbsdyFisc() {
		...
		try {
			...
			/*為了 getFEPTXN2280Count ，取得 BSDAYS_DATE */
