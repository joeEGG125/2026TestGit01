...
import com.syscom.fep.jms.instance.map.onl.ONLQueueConfiguration;
import com.syscom.fep.jms.queue.ONLQueueConsumers;
import com.syscom.fep.server.common.TxHelper;
import com.syscom.fep.server.common.handler.ONLHandler;
...
@StackTracePointCut(caller = SvrConst.SVR_ONL)
public class ONLServerReceiver extends FEPBase implements JmsReceiver<String> {

    @PostConstruct
    public void initialization() {
        JmsMonitorController.addNotifier(ProgramName, SpringBeanFactoryUtil.registerBean(ONLQueueConsumers.class).subscribe(this));
    }
    ...
    public void messageReceived(String destination, String payload, Message message) {
        ...
        try {
            ...
            logData.setRemark("ONL Service Receive Request");
    ...
    private String MultiProcess(LogData logData, String messageIn, Message message) {
        ...
        try {
            ...
            logData.setProgramException(e);
            logData.setServiceUrl("/onl/recv");
            this.logMessage(logData);
            // sendEMS
            logData.setProgramName(StringUtils.join(ProgramName, ".sendReceive"));
            logData.setProgramException(e);
...
//            sender.sendQueue(configuration.getQueueNames().getOnlAck().getDestination(), returnStr, null, new JmsHandler() {
//                @Override
...
//            });
            ...
            ONLQueueConfiguration ONLQueueConfiguration = SpringBeanFactoryUtil.getBean(ONLQueueConfiguration.class);
            MapQueueOperator<?, ?> operator = ONLQueueConfiguration.getMapQueueOperator(replyTo);
            if (operator != null) {
                JmsDefinition ackDefinition = ONLQueueConfiguration.getONLAck(replyTo);
