...
public class EATMController extends BaseController {
    ...
    @RequestMapping(value = "/recv/eatm", method = RequestMethod.POST, produces = MediaType.APPLICATION_XML_VALUE)
    @ResponseBody
    ...
    protected String processRequestData(ProgramFlow programFlow, String messageIn) {
        ...
        try {
            ...
            logData.setRemark(this.getName() + " Get Response from EATMHandler");
            this.logMessage(logData);
        } catch (Exception e) {
            logData.setMessage(messageIn);
            logData.setProgramName(StringUtils.join(ProgramName, ".processRequestData"));
