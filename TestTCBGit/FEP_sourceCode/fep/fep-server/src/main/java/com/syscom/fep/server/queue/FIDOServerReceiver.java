...
import com.syscom.fep.jms.instance.map.fido.FIDOQueueConfiguration;
import com.syscom.fep.jms.queue.FIDOQueueConsumers;
import com.syscom.fep.server.common.TxHelper;
import com.syscom.fep.server.common.handler.FIDOHandler;
...
@StackTracePointCut(caller = SvrConst.SVR_FIDO)
public class FIDOServerReceiver extends FEPBase implements JmsReceiver<String> {

    @PostConstruct
    public void initialization() {
        JmsMonitorController.addNotifier(ProgramName, SpringBeanFactoryUtil.registerBean(FIDOQueueConsumers.class).subscribe(this));
    }
    ...
    public void messageReceived(String destination, String payload, Message message) {
        ...
        try {
            ...
            logData.setRemark("FIDO Service Receive Request");
    ...
    private String MultiProcess(LogData logData, String messageIn, Message message) {
        ...
        try {
            ...
            logData.setProgramException(e);
            logData.setServiceUrl("/fido/recv");
            this.logMessage(logData);
            // sendEMS
            logData.setProgramName(StringUtils.join(ProgramName, ".sendReceive"));
            logData.setProgramException(e);
    ...
    private boolean FeeBackSuccess(LogData logData, String returnStr, Message messageIn) {
        try {
            ...
            // sender.sendQueue(configuration.getQueueNames().getFidoAck().getDestination(), returnStr, null, new JmsHandler() {
            //     @Override
            ...
            FIDOQueueConfiguration fidoQueueConfiguration = SpringBeanFactoryUtil.getBean(FIDOQueueConfiguration.class);
            MapQueueOperator<?, ?> operator = fidoQueueConfiguration.getMapQueueOperator(replyTo);
            if (operator != null) {
                JmsDefinition ackDefinition = fidoQueueConfiguration.getFidoAck(replyTo);
