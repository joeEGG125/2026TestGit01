...
@StackTracePointCut(caller = SvrConst.SVR_RM)
public class RMController extends BaseController {

    @Override
    public String getName() {
        return SvrConst.SVR_RM;
    }

    @RequestMapping(value = "/recv/rm", method = RequestMethod.POST, produces = MediaType.APPLICATION_XML_VALUE)
    @ResponseBody
    ...
    protected String processRequestData(final ProgramFlow programFlow, final String messageIn) {
        ...
        try {
            ...
            messageOut = rmHandler.dispatch(FEPChannel.BRANCH, messageIn);
            logData.setProgramFlowType(this.getOutProgramFlow(programFlow));
            logData.setMessageFlowType(MessageFlow.Response);
            logData.setMessage(messageOut);
            logData.setRemark(this.getName() + " Get Response from RMHandler");
            logData.setChannel(FEPChannel.BRANCH);
            this.logMessage(logData);
        } catch (Exception e) {
            logData.setMessage(messageIn);
