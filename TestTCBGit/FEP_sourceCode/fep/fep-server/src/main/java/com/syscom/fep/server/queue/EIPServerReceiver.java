...
import com.syscom.fep.jms.instance.map.eip.EIPQueueConfiguration;
...
public class EIPServerReceiver extends FEPBase implements JmsReceiver<String> {
    ...
    private String MultiProcess(LogData logData, String messageIn, Message message) {
        ...
        try {
            ...
            eipHandler.setReceivedTime(JmsFactory.getTimestamp(message));
            messageOut = eipHandler.dispatch(FEPChannel.EIP, messageIn);
            logData.setProgramName(StringUtils.join(ProgramName, ".sendReceive"));
...
//            });
            ...
            EIPQueueConfiguration eipQueueConfiguration = SpringBeanFactoryUtil.getBean(EIPQueueConfiguration.class);
            MapQueueOperator<?, ?> operator = eipQueueConfiguration.getMapQueueOperator(replyTo);
            if (operator != null) {
                JmsDefinition ackDefinition = eipQueueConfiguration.getEipAck(replyTo);
