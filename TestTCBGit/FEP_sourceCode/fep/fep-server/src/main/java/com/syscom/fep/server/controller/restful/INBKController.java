...
import com.syscom.fep.server.common.handler.INBKHandler;
 ...
 * @author zk
 */
@StackTracePointCut(caller = SvrConst.SVR_INBK)
public class INBKController extends BaseController {

    @Override
    public String getName() {
        return SvrConst.SVR_INBK;
    }

    @RequestMapping(value = "/recv/inbk", method = RequestMethod.POST, produces = MediaType.APPLICATION_XML_VALUE)
    @ResponseBody
    ...
    protected String processRequestData(final ProgramFlow programFlow, final String messageIn) {
        ...
        logData.setRemark(StringUtils.join(this.getName(), " Receive Request"));
        ...
        try {
            ...
            INBKHandler inbkHandler = new INBKHandler();
            inbkHandler.setEj(logData.getEj());
            inbkHandler.setLogContext(logData);
            messageOut = inbkHandler.dispatch(FEPChannel.BRANCH, messageIn);
            logData.setProgramName(StringUtils.join(ProgramName, ".processRequestData"));
            logData.setProgramFlowType(this.getOutProgramFlow(programFlow));
            logData.setMessageFlowType(MessageFlow.Response);
            logData.setMessage(messageOut);
            logData.setRemark(StringUtils.join(this.getName(), " Get Response from INBKHandler"));
            logData.setChannel(FEPChannel.BRANCH);
            this.logMessage(logData);
        } catch (Exception e) {
            logData.setMessage(messageIn);
