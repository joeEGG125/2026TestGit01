...
public class VOController extends BaseController {
    ...
    @RequestMapping(value = "/recv/vo", method = RequestMethod.POST, produces = MediaType.APPLICATION_XML_VALUE)
    @ResponseBody
    ...
    protected String processRequestData(final ProgramFlow programFlow, final String messageIn) {
        ...
        logData.setChannel(FEPChannel.VO);
        ...
        try {
            ...
            logData.setRemark(this.getName() + " Get Response from VOHandler");
            logData.setChannel(FEPChannel.VO);
            this.logMessage(logData);
        } catch (Exception e) {
            logData.setMessage(messageIn);
