...
import com.syscom.fep.jms.instance.map.digital.DIGITALQueueConfiguration;
import com.syscom.fep.jms.queue.DigitalQueueConsumers;
import com.syscom.fep.server.common.TxHelper;
import com.syscom.fep.server.common.handler.DigitalHandler;
...
@StackTracePointCut(caller = SvrConst.SVR_DIGITAL)
public class DigitalServerReceiver extends FEPBase implements JmsReceiver<String> {

    @PostConstruct
    public void initialization() {
        JmsMonitorController.addNotifier(ProgramName, SpringBeanFactoryUtil.registerBean(DigitalQueueConsumers.class).subscribe(this));
    }
    ...
    public void messageReceived(String destination, String payload, Message message) {
        ...
        try {
            ...
            logData.setRemark("Digital Service Receive Request");
            logData.setServiceUrl("/Digital/recv");
...
//            sender.sendQueue(configuration.getQueueNames().getDigitalAck().getDestination(), returnStr, null, new JmsHandler() {
//                @Override
...
//            });
            ...
            DIGITALQueueConfiguration digitalQueueConfiguration = SpringBeanFactoryUtil.getBean(DIGITALQueueConfiguration.class);
            MapQueueOperator<?, ?> operator = digitalQueueConfiguration.getMapQueueOperator(replyTo);
            if (operator != null) {
                JmsDefinition ackDefinition = digitalQueueConfiguration.getDigitalAck(replyTo);
