...
import com.syscom.fep.jms.instance.map.mch.MCHQueueConfiguration;
...
public class MCHServerReceiver extends FEPBase implements JmsReceiver<String> {
    ...
    public void messageReceived(String destination, String payload, Message message) {
        ...
        try {
            ...
            logData.setMessage("correlationID: " + correlationID + " ,Msgid: " + MsgidFromXml);
            //logData.setMessage("correlationID: " + correlationID + " ,Msgid: " + Msgid);
            logData.setRemark("MCH Service Receive Request");
    ...
    private String MultiProcess(LogData logData, String messageIn, Message message) {
        ...
        try {
            ...
            MCHHandler handler = new MCHHandler();
            ...
            logData.setRemark("MCHService Get Response from MCHHandler");
            this.logMessage(logData);
            return messageOut;
        } catch (Exception e) {
            logData.setMessage(messageIn);
            ...
            logData.setProgramException(e);
            logData.setServiceUrl("/mch/recv");
            this.logMessage(logData);
            // sendEMS
            logData.setProgramName(StringUtils.join(ProgramName, ".sendReceive"));
            logData.setProgramException(e);
    ...
    private boolean FeeBackSuccess(LogData logData, String returnStr, Message messageIn) {
        try {
            ...
            // sender.sendQueue(configuration.getQueueNames().getMchAck().getDestination(), returnStr, null, new JmsHandler() {
            //     @Override
            ...
            MCHQueueConfiguration mchQueueConfiguration = SpringBeanFactoryUtil.getBean(MCHQueueConfiguration.class);
            MapQueueOperator<?, ?> operator = mchQueueConfiguration.getMapQueueOperator(replyTo);
            if (operator != null) {
                JmsDefinition ackDefinition = mchQueueConfiguration.getMchAck(replyTo);
