...
import com.syscom.fep.jms.instance.map.bizloan.BIZLOANQueueConfiguration;
import com.syscom.fep.jms.queue.BizloanQueueConsumers;
import com.syscom.fep.server.common.TxHelper;
import com.syscom.fep.server.common.handler.BizloanHandler;
...
@StackTracePointCut(caller = SvrConst.SVR_BIZLOAN)
public class BizloanServerReceiver extends FEPBase implements JmsReceiver<String> {

    @PostConstruct
    public void initialization() {
        JmsMonitorController.addNotifier(ProgramName, SpringBeanFactoryUtil.registerBean(BizloanQueueConsumers.class).subscribe(this));
    }
    ...
    public void messageReceived(String destination, String payload, Message message) {
        ...
        try {
            ...
            logData.setRemark("Bizloan Service Receive Request");
            logData.setServiceUrl("/bizloan/recv");
    ...
    private String MultiProcess(LogData logData,String messageIn, Message message) {
        ...
        logData.setRemark("BizloanService Receive Request");
        logData.setServiceUrl("/bizloan/recv");
...
//            sender.sendQueue(configuration.getQueueNames().getBizloanAck().getDestination(), returnStr, null, new JmsHandler() {
//                @Override
...
//            });
            ...
            BIZLOANQueueConfiguration bizloanQueueConfiguration = SpringBeanFactoryUtil.getBean(BIZLOANQueueConfiguration.class);
            MapQueueOperator<?, ?> operator = bizloanQueueConfiguration.getMapQueueOperator(replyTo);
            if (operator != null) {
                JmsDefinition ackDefinition = bizloanQueueConfiguration.getBizloanAck(replyTo);
