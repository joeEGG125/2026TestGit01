...
import com.syscom.fep.jms.instance.map.vip.VIPQueueConfiguration;
...
@StackTracePointCut(caller = SvrConst.SVR_VIP)
...
//            }
    ...
    private String MultiProcess(LogData logData, String messageIn) {
        ...
        try {
            ...
            replyTo = vipHandler.getReplyTo();
            messageOut = vipHandler.dispatch(FEPChannel.VIP, messageIn);
            logData.setProgramName(StringUtils.join(ProgramName, ".sendReceive"));
    ...
    private boolean FeeBackSuccess(LogData logData, String returnStr, Message messageIn, String replyTo) {
        try {
            VIPQueueConfiguration vipQueueConfiguration = SpringBeanFactoryUtil.getBean(VIPQueueConfiguration.class);
            MapQueueOperator<?, ?> operator = vipQueueConfiguration.getMapQueueOperator(replyTo);
            if (operator != null) {
                JmsDefinition ackDefinition = vipQueueConfiguration.getVipAck(replyTo);
...
//            List<Npsdtl> alld = npsdtlExtMapper.GetNPSDTLByBATNOforAll(batNo);
            ...
            logData.setRemark(StringUtils.join("prepareReq with Exception occur, ", ex.getMessage()));
            logData.setProgramException(ex);
            sendEMS(logData);
            return null;
        }
