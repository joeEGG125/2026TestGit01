...
import com.syscom.fep.jms.instance.map.hce.HCEQueueConfiguration;
...
public class HCEServerReceiver extends FEPBase implements JmsReceiver<String> {
    ...
    private String MultiProcess(LogData logData, String messageIn, Message message) {
        ...
        try {
            ...
            hceHandler.setReceivedTime(JmsFactory.getTimestamp(message));
    ...
    private boolean FeeBackSuccess(LogData logData, String returnStr, Message messageIn) {
        try {
            ...
            HCEQueueConfiguration hceQueueConfiguration = SpringBeanFactoryUtil.getBean(HCEQueueConfiguration.class);
            MapQueueOperator<?, ?> operator = hceQueueConfiguration.getMapQueueOperator(replyTo);
            if (operator != null) {
                JmsDefinition ackDefinition = hceQueueConfiguration.getHceAck(replyTo);
