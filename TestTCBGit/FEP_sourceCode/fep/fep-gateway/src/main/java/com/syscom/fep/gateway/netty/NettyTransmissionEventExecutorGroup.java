...
public class NettyTransmissionEventExecutorGroup extends AbstractEventExecutorGroup {
    private static final int MAX_LOOPS = Integer.MAX_VALUE; // 2024-09-11 Richard add for 【Unchecked Input for Loop Condition】
    private final List<EventExecutor> children;
    private final AtomicInteger terminatedChildren = new AtomicInteger();
    private final Promise<?> terminationFuture = new DefaultPromise<>(GlobalEventExecutor.INSTANCE);
    private final NettyTransmissionEventExecutorChooserFactory chooserFactory;
    ...
    public NettyTransmissionEventExecutorGroup(int nThreads, long idleCheckInterval) {
        this(nThreads, idleCheckInterval, null);
    }

    public NettyTransmissionEventExecutorGroup(int nThreads, long idleCheckInterval, ThreadFactory threadFactory) {
        this(nThreads, idleCheckInterval, threadFactory, NettyTransmissionEventExecutor.DEFAULT_MAX_PENDING_EXECUTOR_TASKS, RejectedExecutionHandlers.reject());
    }

    public NettyTransmissionEventExecutorGroup(int nThreads, long idleCheckInterval, ThreadFactory threadFactory, int maxPendingTasks, RejectedExecutionHandler rejectedHandler) {
        this(nThreads, idleCheckInterval, threadFactory, new Object[] {maxPendingTasks, rejectedHandler});
    }

    protected NettyTransmissionEventExecutorGroup(int nThreads, long idleCheckInterval, ThreadFactory threadFactory, Object... args) {
        this(nThreads, idleCheckInterval, threadFactory == null ? null : new ThreadPerTaskExecutor(threadFactory), args);
    }

    protected NettyTransmissionEventExecutorGroup(int nThreads, long idleCheckInterval, Executor executor, Object... args) {
        this(nThreads, idleCheckInterval, executor, NettyTransmissionEventExecutorChooserFactory.INSTANCE, args);
    }

    protected NettyTransmissionEventExecutorGroup(int nThreads, long idleCheckInterval, Executor executor, NettyTransmissionEventExecutorChooserFactory chooserFactory, Object... args) {
    ...
    private void addNewChildExecutor(int nThreads) {
        synchronized (this.children) {
            // 2024-09-11 Richard add start for 【Unchecked Input for Loop Condition】
            if (nThreads > MAX_LOOPS) {
                nThreads = MAX_LOOPS;
            }
            // 2024-09-11 Richard add end for 【Unchecked Input for Loop Condition】
    ...
    protected ThreadFactory newDefaultThreadFactory() {
        return new NettyTransmissionThreadFactory(getClass());
    }
    ...
    protected EventExecutor newChild(Executor executor, Object... args) throws Exception {
        return new NettyTransmissionEventExecutor(this, executor, (Integer) args[0], (RejectedExecutionHandler) args[1]);
    }
    ...
    public boolean setExecutorThreads(int nThreads) {
        synchronized (this.children) {
            ...
            int val = MathUtil.safeGet(nThreads - this.children.size());
            // 如果設定的線程數比現有的多, 則需要新增
            if (val > 0) {
            ...
            // 減少線程數
                ...
                for (EventExecutor l : this.children) {
                    ...
                    if (((NettyTransmissionEventExecutor) l).hasBindChannelId()) {
                        kept.add(l);
                        continue;
                    }
                ...
                int remaining = MathUtil.safeGet(nThreads - this.children.size());
                // 如果線程數不夠, 則繼續新增
                if (remaining > 0) {
    ...
    public int executorActiveCount() {
        ...
        synchronized (this.children) {
            for (EventExecutor l : this.children) {
                if (((NettyTransmissionEventExecutor) l).getState() == NettyTransmissionEventExecutor.ST_STARTED)
                    activeCount++;
            }
        }
        return activeCount;
                ...
                synchronized (children) {
                    if (children.size() > nThreads) {
                        ...
                        while (it.hasNext() && children.size() > nThreads) {
                            NettyTransmissionEventExecutor executor = (NettyTransmissionEventExecutor) it.next();
