...
function usage {
  ...
  echo "Usage: $0 {start|stop|check|startChannel primary/secondary/all|stopChannel primary/secondary/all|showlog tx/rsm/disconn/txcurrent}"
...
function showlog {
  java -Dfile.encoding=UTF-8 -cp $jarFile $programName -f showlog -d "logKind=$1&date=$2"
}
...
disconn)
  showlog disconn $2 $3 $4
  ;;

txcurrent)
  showlog txcurrent "" $2 $3
  ;;
