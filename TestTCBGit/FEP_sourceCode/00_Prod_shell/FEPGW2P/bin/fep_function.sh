...
#    local logFilename=$appDirName-all
    ...
    if [[ -d "$appLogsPath" ]];then
        ...
        echo "The $procName is starting on host $hn, please wait..."
...
#nohup java $AppD -jar -Dfile.encoding=UTF-8 $appJarPath > /dev/null 2>&1 &
        ...
        if [ $count != 0 ];then
                ...
                if [[ -z $result ]];then
                        ...
                        echo "The $procName started successfully on host $hn with pid = [$appId]"
                fi
        else
...
function Stop
{
    ...
    if [ $name = "batch.jar" ]; then
        ...
        if [[ -z $appId ]];then
                ...
                echo "The $procName is stopping on host $hn, please wait..."
        kill $appId
                ...
                if [[ -z $appId ]];then
                                echo "The $procName stopped successfully on host $hn"
                                exit 0
                fi
